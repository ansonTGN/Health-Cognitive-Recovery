...
impl KGRepository for Neo4jRepo {
    ...
    async fn export_full_knowledge_graph(&self) -> Result<crate::domain::models::ExportedGraph, AppError> {
        // 1. Obtener todos los nodos con su etiqueta principal
        ...
        while let Ok(Some(row)) = stream_nodes.next().await {
            nodes.push(crate::domain::models::GraphEntity {
                name: row.get("n.name").unwrap_or_default(),
                category: row.get("n.category").unwrap_or("Unknown".to_string()),
            });
        }
        ...
        while let Ok(Some(row)) = stream_edges.next().await {
            edges.push(crate::domain::models::GraphRelation {
                source: row.get("n.name").unwrap_or_default(),
                target: row.get("m.name").unwrap_or_default(),
                relation_type: row.get("type(r)").unwrap_or_default(),
            });
        }

        Ok(crate::domain::models::ExportedGraph {
            generated_at: chrono::Utc::now().to_rfc3339(),
            domain: "Mental Health & Community Recovery".to_string(),
            nodes,
            edges,
        })
    }
